...
namespace Evadne {
	...
	void EditorLayer::OnAttach()
	{
		...
		m_BackgroundTexture = Texture2D::Create("assets/textures/IMG_0467.jpg");
	...
	void EditorLayer::OnImGuiRender()
	{
		...
		else
		{
			...
			uint32_t textureID = m_BackgroundTexture->GetRendererID();
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<MouseButtonPressedEvent>(EV_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	void EditorLayer::OnOverlayRender()
	{
		...
		{
			Entity camera = m_ActiveScene->GerPrimaryCameraEntity();
	...
	{
		std::string filepath = FileDialogs::OpenFile("Evadne Scene (*.escene)\0*.escene\0");
		if (!filepath.empty())
		{
			if (m_SceneState != SceneState::Edit)
				OnSceneStop();
	...
	void EditorLayer::OpenScene(const std::filesystem::path& path)
	{
		...
		if (path.extension().string() != ".escene")
		{
			EV_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::string filepath = FileDialogs::SaveFile("Evadne Scene (*.escene)\0*.escene\0");
		if (!filepath.empty())
		{
	...
	void EditorLayer::UI_Toolbar()
	{
		...
		
		{
			...
			if (ImGui::ImageButton("run_button", (ImTextureID)icon->GetRendererID(), ImVec2(size, size),
	...
	{
		EV_CORE_ASSERT(m_SceneState == SceneState::Play || m_SceneState == SceneState::Simulate);
