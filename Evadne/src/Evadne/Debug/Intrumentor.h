		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession) 
			{
				...
				if (Log::GetCoreLogger()) 
				{
					...
					EV_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name, m_CurrentSession->Name);
			...
			else 
			{
				...
				if (Log::GetCoreLogger()) 
				{
					EV_CORE_ERROR("Instrumentor could not open results file '{0}'.", filepath);
				}
...
#define EV_PROFILE 0
			...
			#define EV_FUNC_SIG __PRETTY_FUNCTION__
		#elif defined(__DMC__) && (__DMC__ >= 0x810)
			#define EV_FUNC_SIG __PRETTY_FUNCTION__
		#elif (defined(__FUNCSIG__) || (_MSC_VER))
			#define EV_FUNC_SIG __FUNCSIG__
		#elif (defined(__INTEL_COMPILER) && (__INTEL_COMPILER >= 600)) || (defined(__IBMCPP__) && (__IBMCPP__ >= 500))
			#define EV_FUNC_SIG __FUNCTION__
		#elif defined(__BORLANDC__) && (__BORLANDC__ >= 0x550)
			#define EV_FUNC_SIG __FUNC__
		#elif defined(__STDC_VERSION__) && (__STDC_VERSION__ >= 199901)
			#define EV_FUNC_SIG __func__
		#elif defined(__cplusplus) && (__cplusplus >= 201103)
			#define EV_FUNC_SIG __func__
		#else
			#define EV_FUNC_SIG "EV_FUNC_SIG unknown!"
		#endif
