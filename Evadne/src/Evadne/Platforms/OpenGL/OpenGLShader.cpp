...
Evadne::OpenGLShader::OpenGLShader(const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc)
	:m_Name(name)
{
...
Evadne::OpenGLShader::OpenGLShader(const std::string& filepath)
{
...
std::string Evadne::OpenGLShader::ReadFile(const std::string& filepath)
{
	std::string result;
...
std::unordered_map<GLenum, std::string> Evadne::OpenGLShader::PreProcess(const std::string& source)
{
	...
	while (pos != std::string::npos)
	{
		...
		EV_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
...
void Evadne::OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
{
	GLuint program = glCreateProgram();
	EV_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now");
	...
	for (auto& kv : shaderSources)
	{
		...
		if (isCompiled == GL_FALSE)
		{
			...
			EV_CORE_ASSERT(false, "Shader compilation failure!");
			break;
		}
